...
2. Users → Add user → the network's address, and a password generated by a
   password manager. Store it there. It is not written down anywhere in this
   repository, and nothing in the build reads it.
3. `gcloud auth application-default login` (once per machine), then
   `npm run grant-admin -- contacto@redinnovacionfrugal.lat`.
4. Open `/admin` and sign in.

To rotate: change the password in the console. To revoke someone entirely:
`npm run grant-admin -- <address> --revoke`.
...
Translated so far: sign-in, the header, the tabs, and the requests queue
(`t.admin.*` in `src/i18n/translations.ts`). The four content editors —
Iniciativas, Documentos, Bibliografía, Congreso — are still Spanish-only.
...
carried all three languages and the queue was reading `.es` off them regardless.
...
|---|---|---|
| Anonymous | 5,000 | ~29 initiative cards, or 3× the whole site's content |
| With `VITE_TRANSLATE_CONTACT` | 50,000 | ~290 cards a day |
...
`VITE_TRANSLATE_CONTACT` is MyMemory's `de=` parameter. It lives in the
environment rather than in source because **this repository is public**: an
address committed here is in the history of every clone and fork permanently and
cannot be rotated. Vite still inlines it into the bundle, so whatever is set is
publicly readable on the deployed site and *will* be scraped — use a throwaway
address, never the network's real inbox. It does have to receive mail: MyMemory
asks for one so they can make contact if the traffic looks wrong, and the answer
to an address that bounces is to block it. Leaving it unset is fine and just
means the anonymous quota.

**Why not Google Cloud Translation or DeepL.** Cloud Translation requires a
billing account even on its free tier, which would take the project off the
Spark plan. DeepL's free tier needs an API key — and a static site with no
server cannot hold one, so it would ship inside the JavaScript for anyone to
read and burn. Both would also need a Cloud Function to proxy, which forces
Blaze. The two providers above need no key and no account, so there is nothing
to leak and no card to charge.
